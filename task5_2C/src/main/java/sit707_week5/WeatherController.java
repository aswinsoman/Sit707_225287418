...
public class WeatherController {
	...
	/*
	 * Clock supplier – returns the current Date.
	 * Default behaviour is the real system clock.
	 * Tests can override this to inject a fixed/fake clock,
	 * making the test Repeatable (FI[R]ST).
	 */
	private java.util.function.Supplier<Date> clockSupplier = Date::new;
	
	/**
	 * @param supplier a Supplier that returns the Date to use
	 */
	public void setClock(java.util.function.Supplier<Date> supplier) {
	...
	public String persistTemperature(int hour, double temperature) {
		...
		// Use the clock supplier instead of new Date() directly
		String savedTime = sdf.format(clockSupplier.get());
